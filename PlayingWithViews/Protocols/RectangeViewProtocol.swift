    ...
    func didEdgeFrameChanged(_ translation: CGPoint, edgeType: EdgeType) {
        ...
        case .leftTop:
            let difx = translation.x,
                dify = translation.y,
                newOrigin = CGPoint(x: initialFrame.origin.x + difx, y: initialFrame.origin.y + dify),
        ...
        case .rightTop:
            let difx = translation.x,
                dify = translation.y,
                newOrigin = CGPoint(x: initialFrame.origin.x, y: initialFrame.origin.y + dify),
                height = initialFrame.height - dify,
                width = initialFrame.width + difx
            newFrame = CGRect(origin: newOrigin, size: CGSize(width: width, height: height))
        case .leftBottom:
            let difx = translation.x,
                dify = translation.y,
                newOrigin = CGPoint(x: initialFrame.origin.x + difx, y: initialFrame.origin.y),
                height = initialFrame.height + dify,
                width = initialFrame.width - difx
            newFrame = CGRect(origin: newOrigin, size: CGSize(width: width, height: height))
        case .rightBottom:
            let difx = translation.x,
                dify = translation.y,
                newOrigin = CGPoint(x: initialFrame.origin.x, y: initialFrame.origin.y),
                height = initialFrame.height + dify,
                width = initialFrame.width + difx
            newFrame = CGRect(origin: newOrigin, size: CGSize(width: width, height: height))
        }
